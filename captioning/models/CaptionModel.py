...
class CaptionModel(nn.Module):
    ...
    def beam_search(self, init_state, init_logprobs, *args, **kwargs):
        ...
        beam_seq_logprobs_table = [torch.FloatTensor(batch_size, bdash, 0, self.vocab_size + 4).to(device) for _ in range(group_size)]
    ...
    def old_beam_search(self, init_state, init_logprobs, *args, **kwargs):
        ...
        beam_seq_logprobs_table = [torch.FloatTensor(self.seq_length, bdash, self.vocab_size + 4).zero_() for _ in range(group_size)]
    ...
    def sample_next_word(self, logprobs, sample_method, temperature, train_mode='AIC'):
        if sample_method == 'greedy':
            if train_mode == 'AIC' or self.train_mode == 'auto':
                sampleLogprobs, it = torch.max(logprobs.data, 1)
                it = it.view(-1).long()
            elif train_mode == 'PB_pad' or train_mode == 'NAIC' or train_mode == 'SAIC' or self.train_mode == 'UIC':
                sampleLogprobs, it = torch.max(logprobs.data, 2)
                it = it.long()
            ...
            if sample_method.startswith('top'): # topk sampling
                ...
                if 0 < top_num < 1:
                    ...
                    logprobs = tmp
            if True in logprobs.isnan():
                print("nan Occured!")
            new_logprobs = logprobs.clone().masked_fill(logprobs.isnan(), float('-10'))
            # try:
            it = torch.distributions.categorical.Categorical(logits=new_logprobs.detach()).sample()
                # for PB_pad
            if train_mode == 'AIC' or self.train_mode == 'auto':
                sampleLogprobs = logprobs.gather(1, it.unsqueeze(1))
            elif train_mode == 'PB_pad' or train_mode == 'NAIC' or train_mode == 'SAIC'  or self.train_mode == 'UIC':
                sampleLogprobs = logprobs.gather(2, it.unsqueeze(2)).squeeze(2) # gather the logprobs at sampled positions
                    # p, id = torch.max(logprobs[0].data, 1)
                    # print(p)
                # print(it.shape, sampleLogprobs.shape)
            # except Exception as e:
            #     return self.sample_next_word(logprobs, 'greedy', float(1.0), train_mode)
    ...
    def init_from(self, state_dict: Union[Dict[str, Tensor], Dict[str, Tensor]],
                        strict: bool = True):
        r"""
        for PBIC init from AIC
        ...
        if self.train_mode == 'PB_pad' or self.train_mode == 'NAIC' or self.train_mode == 'SAIC' or self.train_mode == 'UIC':
            if 'model.tgt_embed.0.lut.weight' in state_dict:
                state_dict['model.tgt_embed.lut.weight'] = state_dict.pop('model.tgt_embed.0.lut.weight')
            if 'model.tgt_embed.1.pe' in state_dict:
                state_dict['model.pos_embed.pe'] = state_dict.pop('model.tgt_embed.1.pe')
        ...
        def load(module, prefix=''):
            ...
            for name, child in module._modules.items():
                #if child is not None and name != 'length_predictor' and name != 'syn_embed' and name != 'SAIC_gate' and name != 'NAIC_gate':
                if child is not None:
                    load(child, prefix + name + '.')

        def load_encoder(module, prefix=''):
            local_metadata = {} if metadata is None else metadata.get(prefix[:-1], {})
            module._load_from_state_dict(
                state_dict, prefix, local_metadata, False, missing_keys, unexpected_keys, error_msgs)
            for name, child in module._modules.items():
                if child is not None and name == 'encoder':
                    print("Initializing Encoder...")
                    load(child, prefix + name + '.')

        local_metadata = {} if metadata is None else metadata.get('', {})
        self._load_from_state_dict(
            state_dict, '', local_metadata, False, missing_keys, unexpected_keys, error_msgs)
        for name, child in self._modules.items():
                if child is not None and name == 'model':
                    load_encoder(child, name + '.')
