...
def eval_split(model, crit, loader, eval_kwargs={}, inference_mode=''):
    ...
    train_mode = eval_kwargs.get('train_mode', 'AIC')
    KD_file = eval_kwargs.get('KD_file', '')
    VOCAB_LOWER=3
    ...
    if train_mode == 'AIC' or train_mode == 'auto':
        ...
        while True:
            ...
            n = n + len(data['infos']) # len(data['infos']) 
            ...
            with torch.no_grad():
                ...
                seq, seq_logprobs, part_time = model(fc_feats, att_feats, att_masks, opt=tmp_eval_kwargs, mode='sample')
                decode_time += part_time
                seq = seq.data
                entropy = - (F.softmax(seq_logprobs, dim=2) * seq_logprobs).sum(2).sum(1) / ((seq>VOCAB_LOWER).to(seq_logprobs).sum(1)+1)
                perplexity = - seq_logprobs.gather(2, seq.unsqueeze(2)).squeeze(2).sum(1) / ((seq>VOCAB_LOWER).to(seq_logprobs).sum(1)+1)
            
            # Print beam search
            if beam_size > 1 and verbose_beam:
                for i in range(fc_feats.shape[0]):
                    k = data['infos'][i]['id']
                    v = [utils.decode_sequence(model.vocab, _['seq'].unsqueeze(0), limit=3)[0] for _ in model.done_beams[i]]
                    seq_kd_train[k] = v
                    # print('\n'.join([utils.decode_sequence(model.vocab, _['seq'].unsqueeze(0))[0] for _ in model.done_beams[i]]))
                    # print('--' * 10)
            sents = utils.decode_sequence(model.vocab, seq, limit=VOCAB_LOWER)
        ...
        while True:
            ...
            if labels is not None and verbose_loss:
                # forward the model to get loss
                with torch.no_grad():
                    length_N, word_logprob = model(fc_feats, att_feats, labels[..., :-1], att_masks)
                    loss = crit(length_N, word_logprob, labels[..., 1:], masks[..., 1:]).item()
                loss_sum = loss_sum + loss
                loss_evals = loss_evals + 1
            ...
            with torch.no_grad():
                tmp_eval_kwargs = eval_kwargs.copy()
                tmp_eval_kwargs.update({'sample_n': 1})
                seq, seq_logprobs, length_N, part_time = model(fc_feats, att_feats, att_masks, opt=tmp_eval_kwargs, mode='sample')
                decode_time += part_time
                seq = seq.data
                entropy = - (F.softmax(seq_logprobs, dim=2) * seq_logprobs).sum(2).sum(1) / ((seq>VOCAB_LOWER).to(seq_logprobs).sum(1)+1)
                perplexity = - seq_logprobs.gather(2, seq.unsqueeze(2)).squeeze(2).sum(1) / ((seq>VOCAB_LOWER).to(seq_logprobs).sum(1)+1)
            ...
            for k, sent in enumerate(sents):
                ...
                if verbose:
                    print('image %s: %s (predict_N:%s)' %(entry['image_id'], entry['caption'], length_N[k].item()))
        ...
        while True:
            ...
            tmp = [data['fc_feats'], data['att_feats'], data['labels'], data['masks'], data['att_masks'], \
                    data['phrase'], data['phrase_num'], data['phrase_length'], data['phrase_syn'], \
                    data['extend_phrase_syn_seq'], data['extend_phrase_seq'], data['extend_phrase_seq_mask']]
            tmp = [_.to(device) if _ is not None else _ for _ in tmp]
            ...
            if labels is not None and verbose_loss:
                ...
                with torch.no_grad():
                    predict_phrase_length_logprob, predict_phrase_syn_logprob, predict_phrase_logprob = \
                        model(fc_feats, att_feats, labels, att_masks, phrase_num, phrase_length, phrase_syn, extend_phrase_syn_seq, extend_phrase_seq, extend_phrase_seq_mask)
                    loss, length_loss_mean, phrase_loss_mean, syn_loss_mean = crit(predict_phrase_length_logprob, predict_phrase_syn_logprob,\
                        predict_phrase_logprob, phrase_num, phrase_length, phrase_syn, labels)
                    loss = loss.item()
                    # loss = crit(model(fc_feats, att_feats, labels[..., :-1], att_masks), labels[..., 1:], masks[..., 1:]).item()
                loss_sum = loss_sum + loss
                loss_evals = loss_evals + 1
            ...
            with torch.no_grad():
                tmp_eval_kwargs = eval_kwargs.copy()
                tmp_eval_kwargs.update({'sample_n': 1})
                seq, seq_logprobs, p_phrase_num, p_phrase_length, p_phrase_syn, part_time = model(fc_feats, att_feats, att_masks, opt=tmp_eval_kwargs, mode='sample')
                decode_time += part_time
                seq = seq.data
                entropy = - (F.softmax(seq_logprobs, dim=2) * seq_logprobs).sum(2).sum(1) / ((seq>VOCAB_LOWER).to(seq_logprobs).sum(1)+1)
                perplexity = - seq_logprobs.gather(2, seq.unsqueeze(2)).squeeze(2).sum(1) / ((seq>VOCAB_LOWER).to(seq_logprobs).sum(1)+1)
            ...
            for k, sent in enumerate(sents):
                entry = {'image_id': data['infos'][k]['id'],\
                        'caption': sent, 'perplexity': perplexity[k].item(), 'entropy': entropy[k].item()}
                if eval_kwargs.get('dump_path', 0) == 1:
                    entry['file_name'] = data['infos'][k]['file_path']
                predictions.append(entry)
                if eval_kwargs.get('dump_images', 0) == 1:
                ...
                if verbose:
                    print("image {}(p_num: {} p_len: {} p_syn: {}): {}".format(entry['image_id'], p_phrase_num[k].data, p_phrase_length[k].data, p_phrase_syn[k].data, entry['caption']) )
                    # print('image %s(p_num: %s p_len: %s): %s' %(entry['image_id'], entry['phrase_num'], entry['phrase_length'], entry['seq']))
        ...
        while True:
            ...
            if num_images >= 0 and n >= num_images:
                break
    elif train_mode == 'UIC' or train_mode == 'UIC_ds':
        while True:
            data = loader.get_batch(split)
            n = n + len(data['infos']) # len(data['infos']) 
            ...
            if labels is not None and verbose_loss:
                ...
                with torch.no_grad():
                    ...
                    loss, SA_length_loss_mean, SA_phrase_loss_mean, SA_syn_loss_mean, NA_length_loss_mean, NA_phrase_loss_mean, NA_syn_loss_mean = \
                        crit(SA_predict_phrase_length_logprob, SA_predict_phrase_syn_logprob, SA_predict_phrase_logprob, \
                            NA_predict_phrase_length_logprob, NA_predict_phrase_syn_logprob, NA_predict_phrase_logprob, \
                            phrase_num, phrase_length, phrase_syn, labels)
                    loss = loss.item()
                    # loss = crit(model(fc_feats, att_feats, labels[..., :-1], att_masks), labels[..., 1:], masks[..., 1:]).item()
                loss_sum = loss_sum + loss
                loss_evals = loss_evals + 1
            ...
            with torch.no_grad():
                tmp_eval_kwargs = eval_kwargs.copy()
                tmp_eval_kwargs.update({'sample_n': 1})
                tmp_eval_kwargs.update({'train_mode': inference_mode})
                seq, seq_logprobs, p_phrase_num, p_phrase_length, p_phrase_syn, part_time = model(fc_feats, att_feats, att_masks, opt=tmp_eval_kwargs, mode='sample')
                decode_time += part_time
                seq = seq.data
                entropy = - (F.softmax(seq_logprobs, dim=2) * seq_logprobs).sum(2).sum(1) / ((seq>VOCAB_LOWER).to(seq_logprobs).sum(1)+1)
            ...
            if num_images >= 0 and n >= num_images:
                break
    elif train_mode == 'UIC_s' or train_mode == 'UIC_u':
        while True:
            data = loader.get_batch(split)
            n = n + len(data['infos']) # len(data['infos']) 
            ...
            if labels is not None and verbose_loss:
                ...
                with torch.no_grad():
                    ...
                    loss, length_loss_mean, syn_loss_mean, A_phrase_loss_mean, SA_phrase_loss_mean, NA_phrase_loss_mean, SA_KL_loss_mean, NA_KL_loss_mean = \
                        crit(predict_phrase_length_logprob, predict_phrase_syn_logprob, A_predict_phrase_prob, A_predict_phrase_logprob,
                            SA_predict_phrase_prob, SA_predict_phrase_logprob, NA_predict_phrase_logprob,
                            phrase_num, phrase_length, phrase_syn, labels)
                    loss = loss.item()
                    # loss = crit(model(fc_feats, att_feats, labels[..., :-1], att_masks), labels[..., 1:], masks[..., 1:]).item()
        ...
        while True:
            ...
            tmp = [data['fc_feats'], data['att_feats'], data['labels'], data['masks'], data['att_masks'], data['phrase'], data['phrase_num'], data['phrase_length']]
            tmp = [_.to(device) if _ is not None else _ for _ in tmp]
            fc_feats, att_feats, labels, masks, att_masks, phrase, phrase_num, phrase_length = tmp

            if labels is not None and verbose_loss:
                # forward the model to get loss
                with torch.no_grad():
                    predict_phrase_length, predict_phrase_length_logprob, predict_phrase_logprob = model(fc_feats, att_feats, labels, att_masks, phrase_num, phrase_length)
                    loss, length_loss, phrase_loss = crit(phrase[..., 1:], predict_phrase_logprob, masks[..., 1:], phrase_num, phrase_length[..., 1:], predict_phrase_length, predict_phrase_length_logprob)
                    loss = loss.item()
                    # loss = crit(model(fc_feats, att_feats, labels[..., :-1], att_masks), labels[..., 1:], masks[..., 1:]).item()
                loss_sum = loss_sum + loss
                loss_evals = loss_evals + 1
            ...
            with torch.no_grad():
                tmp_eval_kwargs = eval_kwargs.copy()
                tmp_eval_kwargs.update({'sample_n': 1})
                seq, seq_logprobs, p_phrase_num, p_phrase_length, part_time = model(fc_feats, att_feats, att_masks, opt=tmp_eval_kwargs, mode='sample')
                decode_time += part_time
                seq = seq.data
                entropy = - (F.softmax(seq_logprobs, dim=2) * seq_logprobs).sum(2).sum(1) / ((seq>3).to(seq_logprobs).sum(1)+1)
                perplexity = - seq_logprobs.gather(2, seq.unsqueeze(2)).squeeze(2).sum(1) / ((seq>3).to(seq_logprobs).sum(1)+1)
            ...
            if beam_size > 1 and verbose_beam:
                for i in range(fc_feats.shape[0]):
                    print('\n'.join([utils.decode_sequence(model.vocab, _['seq'].unsqueeze(0))[0] for _ in model.done_beams[i]]))
                    print('--' * 10)
            sents = utils.decode_sequence(model.vocab, seq, limit=3)
            ...
            for k, sent in enumerate(sents):
                ...
                if verbose:
                    print("image {}(p_num: {} p_len: {}): {}".format(entry['image_id'], p_phrase_num[k].data, p_phrase_length[k].data, entry['caption']) )
                    # print('image %s(p_num: %s p_len: %s): %s' %(entry['image_id'], entry['phrase_num'], entry['phrase_length'], entry['seq']))
    ...
    print("train_mode:{}  inference_mode:{}  inference_time:{:.3f}".format(train_mode, inference_mode if inference_mode != '' else train_mode, decode_time))
    if (train_mode == 'AIC' or train_mode == 'auto') and KD_file != '':
        fp = os.path.join('KD_dataset/', KD_file)
        json.dump(seq_kd_train, open(fp, 'w'))
    lang_stats = None
